...
import { assignPeaje } from "./actions";

type ServicioOption = { id: string; label: string };

export function AssignPeajeSelect({
  peajeId,
  currentVentaId,
  servicios,
}: {
  ...
  function onChange(e: React.ChangeEvent<HTMLSelectElement>) {
    ...
    startTransition(async () => {
      const res = await assignPeaje(peajeId, next || null);
      if (res.error) {
        setError(res.error);
